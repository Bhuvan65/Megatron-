...
A scroll-driven 3D landing page for a procedural chrome mecha. It's built with React 18, R3F v8, Rapier, Babylon.js and a socket.io telemetry feed.

Developed by Bhuvan and Tony Stark.

```bash
npm i && npm run dev
```

`npm run dev` starts Vite on http://localhost:5173 and the telemetry server on :3001. Vite proxies `/socket.io` to the telemetry server. If the socket can't connect after 3 attempts, the app switches to a local simulator and the HUD shows `LINK SIMULATED` in violet.
...
- At most two WebGL contexts exist at once: R3F switches to `frameloop="never"` while the Babylon hangar is open.
